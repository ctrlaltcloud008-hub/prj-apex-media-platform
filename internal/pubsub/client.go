...
type SubscribetOption func(*pubsub.Subscriber)

func withMaxOutstandingMessages(max int) SubscribetOption {
	return func(s *pubsub.Subscriber) { s.ReceiveSettings.MaxOutstandingMessages = max }
}

func withNumGoroutines(num int) SubscribetOption {
...
func NewSubscriber(client *pubsub.Client, subscriptionID string, opts ...SubscribetOption) *Subscriber {
