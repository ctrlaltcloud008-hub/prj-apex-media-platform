...
func CheckGCSGeneration(ctx context.Context, txn *spanner.ReadWriteTransaction, videoID string, generation int64) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("failed to read video record: %v", err)
	}

	var storedGeneration int64
	if err := row.ColumnByName("gcs_generation", &storedGeneration); err != nil {
		return false, fmt.Errorf("failed to read gcs_generation: %v", err)
	}

	return storedGeneration == generation, nil
}

func TransodeJobID(videoID, profile string, generation int64) (string, error) {
	if videoID == "" {
...
func VertexAIJobName(videoID, profile string, generation int64) (string, error) {
	if videoID == "" {
		return "", fmt.Errorf("videoID cannot be empty")
	}
